...
### Query Builder
ZubZet now includes a [Query Builder](../../core-features/query-builder/index.md) based on [CakePHP\Database](https://api.cakephp.org/5.2/namespace-Cake.Database.html).
This provides a more intuitive and powerful way to create and manage complex MySQL queries.

### Routing
Alongside the existing file-internal routing system ([controller-based actions](../../core-features/controllers-and-actions.md)),
...
1. **Localization System**
  The currently implemented localization system in ZubZet is considered deprecated. While it has served its purpose for handling multiple languages and regional variations, its architecture no longer meets the evolving requirements of the platform. As a result, it will soon be replaced with a more efficient, flexible, and maintainable solution that better aligns with future development goals and user needs.
