...
1. Added - Background tasks: `Tasks::dispatch()` queues a class from `app/Tasks/` (application or module) and returns immediately with a record a frontend polls, while `queue:work --workers=N` runs the work in separate processes. Persistent state lives in `z_task`, outstanding work in `z_task_queue`, claimed with a conditional update that is exclusive across Galera nodes; abandoned claims are recovered after `task_reservation_timeout`. Userspace never writes a query: `Tasks::dispatch/find/forUser/pending` plus `$this->payload` and `$this->progress()` inside a task are the whole surface. Optional owner-scoped status endpoint at `GET /_zubzet/task/{id}` (`task_endpoint_enabled`, default on) exposing progress and timing only. New settings `task_endpoint_enabled`, `task_max_attempts` (default `1`, so retries are opt-in) and `task_reservation_timeout`. Stopping the workers drains them (the task in flight finishes first) when `ext-pcntl` is present and the signal reaches the supervisor: a worker container needs `init: true`, a `stop_grace_period`, and `stop_signal: SIGTERM` when it reuses a web image, since Apache images declare `SIGWINCH`. Without any of that the workers are killed and their tasks recovered through the reservation timeout instead. See the [Background Tasks docs](docs/advanced-features/background-tasks.md)
1. Added - `Connection::$affectedRows` reports how many rows the last statement changed, so a conditional update can tell whether it won
