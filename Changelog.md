...
## v1.4.0
1. Added CSRF protection for `Z.Forms` and `Z.Request`: a host-only `z_csrf` cookie must come back as an `X-CSRF-Token` header on every non-GET request carrying `isFormData` or `_zReq`, otherwise the request is answered with 403. The cookie is `Secure` by default; set `csrf_secure = false` only for a non-HTTPS deployment. **Migrator note:** Z.js attaches the header itself, so `Z.Forms` / `Z.Request` need no change; custom AJAX sending those markers must read the cookie and set the header. Raw `<form method="post">` stays unprotected in this release; the bundled login is covered, since its credential path is reachable only through `$req->isAction("login")`
1. Added `Z.Request.csrfToken()`, which reads the `z_csrf` cookie so custom `fetch()` / `$.ajax()` calls can attach the `X-CSRF-Token` header without parsing `document.cookie` themselves
1. Changed `$req->isAction()` to require the `_zReq` marker that Z.js attaches, instead of matching the `action` field alone. Without it a forged cross-site post could reach any `isAction()` branch by simply omitting the marker, which also skips the CSRF check. **Migrator note:** `Z.Request.action()` / `Z.Request.root()` already send `_zReq`; a custom client posting an `action` straight to a controller has to send `_zReq=1` and the `X-CSRF-Token` header, otherwise the branch is no longer taken
